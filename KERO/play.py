...
async def play(client: Client, message):
  ...
  if message.sender_chat:
     if not message.chat.type == ChatType.CHANNEL:
      return await message.reply_text("لا يمكنك التشغيل بقناة.")
  if not len(message.command) == 1:
    rep = await message.reply_text("جاري التشغيل الان استني شوية.")
  try:
          call = await get_call(bot_username)
  except:
          await remove_active(bot_username, chat_id)
  ...
  if not message.reply_to_message:
     if len(message.command) == 1:
      if message.chat.type == ChatType.CHANNEL:
        return await message.reply_text("أرسل الشيء الذي تريد تشغيله..")
      try:
       name = await client.ask(message.chat.id, text="أسم او رابط الشيء اللذي تريد تشغيله.", reply_to_message_id=message.id, filters=filters.user(message.from_user.id), timeout=200)
       name = name.text
     ...
     if "v" in message.command[0] or "---" in message.command[0]:
       vid = True
     else:
       vid = None 
     ...
     await rep.delete()
       ...
       if not message.reply_to_message.media:
         return
       rep = await message.reply_text("جاري التنزيل انتظر قليلا.") 

...
async def download(bot_username, link, video: Union[bool, str] = None):
    ...
    # yt-dlp options
    # ensure output directory exists and use proper template for filenames
    os.makedirs(bot_username, exist_ok=True)
    if video:
        opts = {
            "cookiefile": "cookies.txt",
            "format": "best[height<=720]",
            "outtmpl": os.path.join(f"{bot_username}", "%(title)s.%(ext)s"),
            "progress_hooks": [hook],
            "noplaylist": True,
            "quiet": True,
            "no_warnings": True,
    ...
    try:
        ...
        print("Download timeout - file may be too large or connection is slow")
    ...
    # If yt-dlp reported a filename that no longer exists (e.g. postprocessor
    # converted m4a -> mp3 and removed the original), try to resolve the
    # actual file on disk by matching the basename in the target directory.
    if downloaded_file:
        try:
            base = os.path.splitext(os.path.basename(downloaded_file))[0]
            for f in os.listdir(bot_username):
                if f.startswith(base):
                    resolved = os.path.abspath(os.path.join(bot_username, f))
                    print("Resolved downloaded file to:", resolved)
                    return resolved
        except Exception:
            pass
...
async def change_stream(bot_username, client, chat_id):
           try:
            ...
            button = [[InlineKeyboardButton(text="• 𝖤𝗇𝖣 •", callback_data=f"stop"), InlineKeyboardButton(text="• 𝖱𝖾𝖲𝗎𝖬𝖾 •", callback_data=f"resume"), InlineKeyboardButton(text="• 𝖯𝖺𝖴𝗌𝖤 •", callback_data=f"pause")], [InlineKeyboardButton(text="• 𝖢𝗁𝖠𝗇𝗇𝖾𝗅 •", url=f"{ch}"), InlineKeyboardButton(text="• 𝖦𝗋𝖴𝗈𝖯 •", url=f"{gr}")], [InlineKeyboardButton(text=f"{OWNER_NAME}", url=f"https://t.me/{OWNER[0]}")], [InlineKeyboardButton(text="• اضف البوت لـ جروبك 🐾.", url=f"https://t.me/{bot_username}?startgroup=True")]]
